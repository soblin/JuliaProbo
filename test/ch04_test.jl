...
@testset "ch04_sim04" begin
    ...
    for i in 1:10
        robot = RealRobot([0.0, 0.0, 0.0], circling_agent, nothing;
                          radius=0.05, color="gray", expected_stuck_time=60.0,
...
@testset "ch04_sim09" begin
    ...
    push!(m, landmarks)
    circling = Agent(0.2, 10.0 / 180 * pi)
    robot = RealRobot([0.0, 0.0, 0.0], circling, RealCamera(landmarks; phantom_prob=0.2); color="red")
    push!(world, robot)
    push!(world, m)
    dt = 0.1
...
@testset "ch04_sim10" begin
    ...
    push!(m, landmarks)
    circling = Agent(0.2, 10.0 / 180 * pi)
    robot = RealRobot([0.0, 0.0, 0.0], circling, RealCamera(landmarks; overlook_prob=0.5); color="red")
    push!(world, robot)
    push!(world, m)
    dt = 0.1
    anim = @animate for i in 1:50
